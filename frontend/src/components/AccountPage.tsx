      ...
      <section className="auth-layout" aria-labelledby="account-title">
        <div className="auth-story">
          ...
          <ul className="auth-benefits">
            <li><ShieldCheck weight="fill" /><span><strong>Verified identity</strong>Secure your orders with our robust backend authentication.</span></li>
        ...
        <div className="auth-card">
          {loading ? (
            ...
            <>
              ...
              <button className="button button-outline-dark auth-secondary" onClick={handleLogout}>Sign out</button>
            </>
          ) : (
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (mode === "login") {
      try {
        ...
        if (!response.ok) { setPassword(""); setMessage(data.error || "Email or password is incorrect."); return; }
        onAuthenticated();
        window.location.assign(data.user?.isAdmin ? "/admin" : "/account");
      } catch (err) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) { setMessage(data.error || "Could not create account."); return; }
      onAuthenticated();
  ...
  return (
    <>
      ...
      <form className="credential-form" noValidate onSubmit={submit}>
        ...
        <p className={`credential-message ${message ? "is-error" : ""}`} role="status">{message || (mode === "signup" ? "Use at least 8 characters." : "Enter your account credentials.")}</p>
